...
int main(void) {
    ...
    // Send messages
    printf("\'task11-2-1.c\' starts sending messages\n");
    for (int i = 0; i < 5; i++) {
        // Message information
        mybuf.mtype = 1;
        mybuf.info.num = i;
        ...
        printf("%d message was sent with information = [%d ; \'%s\']\n", i, mybuf.info.num, mybuf.info.text);
    }
    printf("\'task11-2-1.c\' finished sending messages\n");
    printf("---\n");
    // Reading messages in FIFO
    printf("\'task02-1.c\' started receiving messages\n");
    for (int i = 0; i < 5; i++) {
        max_length = sizeof(mybuf.info);
        // Try to read from queue
        if ((message_length = msgrcv(msqid, (struct msgbuf *) &mybuf, max_length, 0, 0)) < 0) {
            printf("The message has not been received from queue.\n");
            exit(-1);
        }
